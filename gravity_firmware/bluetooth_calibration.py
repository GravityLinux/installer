...
"""Collect BCM4388 beamforming calibration from this Mac's factory data.
...
def validate_blob(data):
    if not 4 <= len(data) <= MAX_CALIBRATION_SIZE or not data.startswith(b"BLOB"):
        raise ValueError("Invalid Bluetooth beamforming calibration")
    return data
...
def extract_btbf(data):
    ...
    def visit(fields, depth=0):
        ...
        for tag, value in fields:
            ...
            if tag == SEQ and children and children[0] == (INT, int.from_bytes(b"BTBF", "little")):
                if len(children) != 3 or children[1][0] != STR or children[2][0] != OCTETS:
                    raise ValueError("Malformed BTBF record")
    ...
    visit(records)
    if len(matches) != 1:
        raise ValueError("Expected exactly one BTBF record")
    return matches[0]
    ...
    def visit(node):
        if isinstance(node, dict):
            if node.get("IORegistryEntryName") == "chosen":
                value = node.get("mac-address-bluetooth0")
...
class BluetoothCalibration:
    ...
    def collect(cls, factory_dir=FACTORY_DIR, tree=None):
        ...
        if not paths:
            log.warning("No J773g Bluetooth factory calibration found; Bluetooth calibration will be unavailable")
            return None
        if tree is None:
            registry = subprocess.run(["ioreg", "-p", "IODeviceTree", "-n", "chosen", "-r", "-a", "-l"],
                                      capture_output=True, check=True)
            tree = plistlib.loads(registry.stdout)
        address = bluetooth_address(tree)
        blobs = {}
        provenance = []
        for path in paths:
            if path.stat().st_size > MAX_FACTORY_SIZE:
                raise ValueError("Factory record exceeds size limit")
            raw = path.read_bytes()
            blob = extract_btbf(raw)
            blobs[hashlib.sha256(blob).hexdigest()] = blob
            provenance.append({"file": path.name, "sha256": hashlib.sha256(raw).hexdigest()})
        if len(blobs) != 1:
            raise ValueError("Multiple different Bluetooth factory calibrations; refusing to choose")
        return cls(address, next(iter(blobs.values())), provenance)

    def files(self):
        name = f"brcm/brcmbt4388-{self.address}-bf.bin"
        return [(name, FWFile(name, self.data))]

    def write_raw(self, directory):
        directory = pathlib.Path(directory)
        (directory / RAW_BLOB).parent.mkdir(parents=True, exist_ok=True)
        (directory / RAW_BLOB).write_bytes(self.data)
        ...
        (directory / RAW_METADATA).write_text(json.dumps(metadata, indent=2) + "\n")

    @classmethod
    def from_archive(cls, directory):
        directory = pathlib.Path(directory)
        metadata_path = directory / RAW_METADATA
        if not metadata_path.exists():
            log.warning("Saved firmware archive has no Bluetooth factory calibration")
            return None
        metadata = json.loads(metadata_path.read_text())
        blob_path = directory / RAW_BLOB
        if blob_path.stat().st_size > MAX_CALIBRATION_SIZE:
            raise ValueError("Saved Bluetooth calibration exceeds size limit")
        data = blob_path.read_bytes()
        if (metadata.get("version") != 1 or metadata.get("bytes") != len(data) or
                metadata.get("sha256") != hashlib.sha256(data).hexdigest()):
            raise ValueError("Saved Bluetooth calibration metadata mismatch")
